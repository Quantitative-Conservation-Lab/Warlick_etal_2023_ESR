...
```{r aerial survey 2022 agTrend modeled count data}
...
start <- 1; stop <- 18
seas <- 'NB' #NB = 'non-breeding' season, fall-spring
seas <- 'sum'
...
#organize data for environmental projection scenarios - used below  
sst_dat <- as.numeric(eval(parse(text = paste0('ocean_SSL$sst_', seas, '[', start,':', stop,']'))))
npgo_dat <- eval(parse(text = paste0('ocean_SSL$NPGO_', seas, '[', start,':', stop,']')))
...
foreach(i = 1:nc) %dopar% { 
  ...
  version <- 'null'
  saveRDS(out, here('results', paste("out.parallel-", version, i, ".RDS", sep = ""))) 
...
#these results not included in the repository
version <- 'null'

out_1 <- readRDS(here::here('results', paste("out.parallel-", version, "1.RDS", sep = "")))
out_2 <- readRDS(here::here('results', paste("out.parallel-", version, "2.RDS", sep = "")))
out_3 <- readRDS(here::here('results', paste("out.parallel-", version, "3.RDS", sep = "")))
...
params <- c('phiP.prob', 'phi1.prob', 'phi2.prob', 'phi3.prob', 'phi4.prob', 'phi5.prob',
            ...
            'b.delB', 'p.delB', 'b.eff', 'eps.p',             'sigma.p', 
...
foreach(i = 1:nc) %dopar% { # parallel version of for loop
  ...
  version <- 'RE'
  saveRDS(out, here('results', paste("out.parallel-", version, "-", i, ".RDS", sep = ""))) # save each chain with a diff name
...
#these results not in the repository
version <- 'RE'
out_1 <- readRDS(here::here('results', paste("out.parallel-", version, "-1.RDS", sep = "")))
out_2 <- readRDS(here::here('results', paste("out.parallel-", version, "-2.RDS", sep = "")))
out_3 <- readRDS(here::here('results', paste("out.parallel-", version, "-3.RDS", sep = "")))
...
```{r random effects and env cov model}
...
```{r age structure posteriors, etc}
...
#summarize phi and psi posteriors
...
#this takes a long time - load in csv below
...
```{r null model projection}

#prep storage, posteriors, etc
...
#rebuild in array format for faster simulation loops
...
out_1 <- readRDS(here::here('results', paste0("out.parallel-env-1.RDS")))
out_2 <- readRDS(here::here('results', paste0("out.parallel-env-2.RDS")))
out_3 <- readRDS(here::here('results', paste0("out.parallel-env-3.RDS")))
...
```{r env covariate projection scenarios}
...
## save objects
# saveRDS(Ntot.sim.cov, file = here::here('results', 'projections', paste0('Ntot_env.rds')))
# saveRDS(lambda.mean.sim.cov, file = here::here('results', 'projections', paste0('lambda_env.rds')))
# saveRDS(p.ext.sim.cov, file = here::here('results', 'projections', paste0('p.ext_env.rds')))
# saveRDS(p.ad.ext.sim.cov, file = here::here('results', 'projections', paste0('p.ad.ext_env.rds')))
# saveRDS(p.neg.sim.cov, file = here::here('results', 'projections', paste0('p.neg_env.rds')))
